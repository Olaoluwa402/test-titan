...
import { createPropertyAction } from "../../../redux/actions/propertiesActions";
import { CREATE_PROPERTY_RESET } from "../../../redux/constants/propertiesConstants";
// import { ToastContainer } from "react-toastify";
import { toast } from "react-toastify";

const NewProperty = ({ setOpen, closeModalHandler }) => {
  //   const welcomeRef = useRef(null);
  const router = useRouter();
  const dispatch = useDispatch();
  const store = useSelector((store) => store.newProperty);
  ...
  const {
    ...
    onSubmit: (values, actions) => {
      ...
      dispatch(createPropertyAction(formData));
      actions.resetForm();
    },
  });
