...
import { createAdvantageAction } from "../../../redux/actions/advantagesActions";
import { CREATE_ADVANTAGE_RESET } from "../../../redux/constants/advantageConstants";
// import { ToastContainer } from "react-toastify";
import { toast } from "react-toastify";

const NewAdvantage = ({ setOpen, closeModalHandler }) => {
  //   const welcomeRef = useRef(null);
  const router = useRouter();
  const dispatch = useDispatch();
  const store = useSelector((store) => store.newAdvantage);
  ...
  const {
    ...
    onSubmit: (values, actions) => {
      ...
      dispatch(createAdvantageAction(formData));
      actions.resetForm();
    },
  });
