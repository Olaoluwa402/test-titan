...
import { createImageAction } from "../../../redux/actions/imagesActions";
import { CREATE_IMAGE_RESET } from "../../../redux/constants/imagesConstants";
// import { ToastContainer } from "react-toastify";
import { toast } from "react-toastify";

const NewImage = ({ setOpen, closeModalHandler }) => {
  //   const welcomeRef = useRef(null);
  const router = useRouter();
  const dispatch = useDispatch();
  const store = useSelector((store) => store.newImage);
  ...
  const {
    ...
    onSubmit: (values, actions) => {
      ...
      dispatch(createImageAction(formData));
      actions.resetForm();
    },
  });
